...
class MedicalRAG:
    ...
    def __init__(self, config):
        ...
        self.grader = RAGGrader(config)  # CRAG / Self-RAG LLM-as-judge graders
    ...
    def process_query(self, query: str, chat_history: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
        ...
        # Preserve the user's original question for CRAG grading (the `query`
        # variable is overwritten by the expanded query below).
        original_query = query

        # CRAG feature switches (safe defaults; overridable via config).
        crag_enabled = getattr(self.config.rag, "enable_crag", True)
        relevance_threshold = getattr(self.config.rag, "crag_relevance_threshold", 0.5)
        ...
        try:
            ...
            # Step 3.5 (CRAG): grade document relevance and decide whether the
            # retrieved context is good enough, or whether we should fall back to
            # web search (via the existing confidence-based routing).
            documents_for_generation = reranked_documents
            crag_needs_web = False
            if crag_enabled and reranked_documents:
                try:
                    filtered = [
                        doc for doc in reranked_documents
                        if self.grader.grade_document_relevance(original_query, doc.get("content", ""))
                    ]
                    relevant_ratio = len(filtered) / len(reranked_documents)
                    self.logger.info(
                        f"3.5 CRAG relevance grading: {len(filtered)}/{len(reranked_documents)} "
                        f"relevant (ratio={relevant_ratio:.2f})"
                    )
                    if filtered:
                        documents_for_generation = filtered
                    if not filtered or relevant_ratio < relevance_threshold:
                        crag_needs_web = True
                except Exception as e:
                    self.logger.warning(f"CRAG document grading skipped ({e}); using reranked docs as-is")

            # Step 3.6: Parent-child (small-to-big) expansion — merge same-source
            # chunks into larger parent context for richer generation (opt-in).
            try:
                from agents.rag_agent.parent_child import expand_to_parents
                documents_for_generation = expand_to_parents(self.config, documents_for_generation)
            except Exception as e:
                self.logger.warning(f"Parent-child expansion skipped ({e})")

            # Step 4: Generate response
            self.logger.info("4. Generating response...")
            response = self.response_generator.generate_response(
                query=query,
                retrieved_docs=documents_for_generation,
                picture_paths=reranked_top_k_picture_paths,
                chat_history=chat_history
                )

            # Step 4.5 (CRAG / Self-RAG): self-reflection — verify the answer is
            # grounded in the retrieved facts (hallucination check) and actually
            # answers the question. On failure, signal a web-search fallback.
            if crag_enabled:
                try:
                    context = "\n\n".join(d.get("content", "") for d in documents_for_generation)
                    answer_text = response.get("response", "") if isinstance(response, dict) else ""
                    grounded = self.grader.grade_hallucination(context, answer_text)
                    addresses = self.grader.grade_answer(original_query, answer_text)
                    self.logger.info(f"4.5 CRAG self-reflection: grounded={grounded}, addresses_question={addresses}")
                    if not grounded or not addresses:
                        crag_needs_web = True
                    response["crag"] = {
                        "grounded": grounded,
                        "addresses_question": addresses,
                        "docs_used": len(documents_for_generation),
                    }
                except Exception as e:
                    self.logger.warning(f"CRAG self-reflection skipped ({e})")

            # If CRAG judged the retrieved context insufficient, drop confidence to
            # 0 so agent_decision.confidence_based_routing re-routes to web search
            # (no graph changes required).
            if crag_needs_web and isinstance(response, dict):
                self.logger.info("CRAG: context insufficient -> lowering confidence to trigger web-search fallback")
