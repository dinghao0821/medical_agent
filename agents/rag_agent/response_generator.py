...
class ResponseGenerator:
    ...
    def generate_response(
        ...
        try:
            ...
            # Security: fence retrieved content so the model treats it as data,
            # not instructions (defends against injection inside documents).
            try:
                from agents.guardrails.injection_filter import wrap_untrusted
                context = wrap_untrusted(self.config, context, source="retrieved_documents")
            except Exception:
                pass
            ...
            # Sentence-level citation grounding + confidence badge (opt-in).
            # Rewrites the answer body with inline [n] markers keyed to the
            # retrieved docs; fail-open leaves the answer untouched.
            answer_body = response.content
            try:
                from agents.rag_agent.citation import add_citations
                cited_body, _cite_meta = add_citations(
                    self.config, answer_body, retrieved_docs, self.response_generator_model
                )
                if cited_body:
                    response.content = cited_body
            except Exception:
                pass
