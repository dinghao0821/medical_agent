...
class VectorStore:
    ...
    def __init__(self, config):
        ...
        # Cross-platform default cache dir (works on Windows/macOS/Linux);
        # can still be overridden via the FASTEMBED_CACHE_PATH env var.
        _default_cache_dir = os.path.join(
            os.path.expanduser("~"), ".cache", "fastembed"
        )
        self.sparse_cache_dir = os.getenv("FASTEMBED_CACHE_PATH", _default_cache_dir)
        self.sparse_model_path = os.getenv(
            "FASTEMBED_BM25_PATH",
            os.path.join(self.sparse_cache_dir, "Qdrant", "bm25"),
        )
    ...
    def _build_sparse_embeddings(self) -> FastEmbedSparse:
        # Force local BM25 path in offline environments to avoid remote fetch attempts.
        kwargs = {}
        if os.path.isdir(self.sparse_model_path):
            kwargs["specific_model_path"] = self.sparse_model_path
            kwargs["local_files_only"] = True
        else:
            self.logger.warning(
                "Local BM25 path not found: %s. FastEmbed may try online download.",
                self.sparse_model_path,
            )
        return FastEmbedSparse(
            model_name="Qdrant/bm25",
            cache_dir=self.sparse_cache_dir,
            **kwargs,
        )
