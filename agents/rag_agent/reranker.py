...
class Reranker:
    ...
    def rerank(self, query: str, documents: Union[List[Dict[str, Any]], List[str]], parsed_content_dir: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract picture references 从重排序后的文档内容中提取图片引用路径，并将这些路径整理成一个列表
            ...
            # Must return a (documents, picture_reference_paths) tuple to match
            # the successful-path signature; otherwise the caller's tuple unpack
            # (reranked_documents, reranked_top_k_picture_paths = rerank(...))
            # would raise a ValueError.
            self.logger.warning("Falling back to original ranking")
            fallback_docs = documents[: self.top_k] if self.top_k else documents
