...
class LocalGuardrails:
    ...
    def check_input(self, user_input: str) -> tuple[bool, str]:
        ...
        """
        # Fail-open: if the guardrail LLM call itself errors (e.g. provider
        # content-inspection 400, timeout), allow the input through rather than
        # blocking the whole request. Downstream agents have their own degradation.
        try:
            result = self.input_guardrail_chain.invoke({"input": user_input})
        except Exception as e:
            logger.warning(f"[Guardrails] Input check failed ({e}); allowing input (fail-open)")
    ...
    def check_output(self, output: str, user_input: str = "") -> str:
        ...
        # Fail-open: the output filter runs on every response, so if its LLM call
        # errors (e.g. provider content-inspection 400 on medical content), return
        # the original output instead of failing the whole request.
        try:
            ...
            logger.warning(f"[Guardrails] Output check failed ({e}); returning unmodified output (fail-open)")
            return output_text
