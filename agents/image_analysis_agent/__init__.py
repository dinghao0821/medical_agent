...
class ImageAnalysisAgent:
    ...
    # brain tumor agent
    def segment_brain_tumor(self, image_path: str, output_path: str = None):
        """Segment a brain MRI. When output_path is None the default (shared) path
        is used; callers should pass a per-session/UUID unique path under
        concurrency to avoid overwriting each other's results."""
        return self.brain_tumor_agent.predict(
            image_path, output_path or self.brain_tumor_segmentation_output_path
        )
    
    # skin lesion agent
    def segment_skin_lesion(self, image_path: str, output_path: str = None) -> str:
        """Segment a skin lesion. When output_path is None the default (shared)
        path is used; callers should pass a per-session/UUID unique path under
        concurrency to avoid overwriting each other's results."""
        return self.skin_lesion_agent.predict(
            image_path, output_path or self.skin_lesion_segmentation_output_path
        )
